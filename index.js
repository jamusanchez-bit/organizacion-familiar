...
<body>
  ...
  <script>
    ...
    function initializeChat() {
      ...
      const greetings = {
        'A1.1': 'Hello! I am Elizabeth. I will help you practice English. What is your name?',
        'A1.2': 'Hi there! I\'m Elizabeth, your English teacher. How are you today?',
        'A2.1': 'Hello! I\'m Elizabeth. I\'m here to help you improve your English. What would you like to talk about?'
      };
      
      const greeting = greetings[userLevel] || greetings['A1.1'];
      addChatMessage('Elizabeth', greeting, true);
    ...
    function startRecording() {
      ...
      recognition.onresult = (event) => {
        ...
        addChatMessage('Tú', transcript, false);
        sendToElizabeth(transcript);
      };
      
      recognition.onerror = () => {
        addChatMessage('System', 'Error en el reconocimiento de voz. Intenta de nuevo.', false);
        stopRecording();
      };
    ...
    function stopRecording() {
      isRecording = false;
      document.getElementById('record-btn').textContent = '🎤 Hablar';
      document.getElementById('record-btn').style.background = '#667eea';
      if (recognition) {
        recognition.stop();
      }
    }
    ...
    function sendToElizabeth(userMessage) {
      chatHistory.push({ role: 'user', content: userMessage });
      
      // Respuestas educativas simuladas hasta configurar OpenAI
      setTimeout(() => {
        const responses = {
          'A1.1': [
            'Very good! Can you tell me more?',
            'I see. What do you like to do?',
            'That is nice. How old are you?',
            'Good! Where do you live?'
          ],
          'A1.2': [
            'That\'s interesting! Can you describe it?',
            'I understand. What else do you enjoy?',
            'Great! Tell me about your family.',
            'Nice! What is your favorite food?'
          ]
        };
        
        const levelResponses = responses[userLevel] || responses['A1.1'];
        const response = levelResponses[Math.floor(Math.random() * levelResponses.length)];
        
        chatHistory.push({ role: 'assistant', content: response });
        addChatMessage('Elizabeth', response, true);
      }, 1500);
    }
