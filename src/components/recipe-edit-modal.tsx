...
import { insertRecipeSchema, type InsertRecipe, type Recipe, type InventoryItem, type RecipeIngredient } from "@shared/schema";
import { z } from "zod";
...
export default function RecipeEditModal({ recipe, isOpen, onClose }: RecipeEditModalProps) {
  ...
  // Fetch recipe ingredients when recipe changes
  const { data: recipeIngredients = [] } = useQuery<(RecipeIngredient & { inventoryItem: InventoryItem })[]>({
    queryKey: ['/api/recipes', recipe?.id, 'ingredients'],
    enabled: !!recipe?.id,
  });

  // Load recipe data when recipe prop changes
  useEffect(() => {
    if (recipe) {
      form.reset({
        name: recipe.name,
        ...
        preparationTime: recipe.preparationTime ? recipe.preparationTime / 60 : undefined, // Convert minutes to hours for display
        crockpotFunction: (recipe.crockpotFunction as "alta" | "baja") || "none",
        servings: recipe.servings || 4,
      });
    }
  }, [recipe, form]);

  // Load recipe ingredients when they're fetched
  useEffect(() => {
    if (recipeIngredients.length > 0) {
      setSelectedIngredients(
        recipeIngredients.map(ri => ({
          ...
          inventoryItem: ri.inventoryItem,
  ...
  const updateRecipeMutation = useMutation({
    mutationFn: async (data: { recipe: InsertRecipe; ingredients: SelectedIngredient[] }) => {
      if (!recipe) throw new Error("No recipe to update");

      // Update recipe first
      await apiRequest('PUT', `/api/recipes/${recipe.id}`, data.recipe);
      ...
      for (const currentIngredient of currentIngredients) {
        if (!data.ingredients.some(ing => ing.id === currentIngredient.id)) {
          await apiRequest('DELETE', `/api/recipe-ingredients/${currentIngredient.id}`);
        }
      }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/recipes', recipe?.id, 'ingredients'] });
      toast({
  ...
  const onSubmit = (data: RecipeFormData) => {
    ...
    const recipeData: InsertRecipe = {
      ...
      preparationTime: data.preparationTime ? Math.round(data.preparationTime * 60) : undefined,
      crockpotFunction: (!data.crockpotFunction || data.crockpotFunction === "none") ? undefined : data.crockpotFunction,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="recipe-edit-modal">
        <DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {selectedIngredients.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="p-6 text-center">
                  <p className="text-neutral-500">No hay ingredientes en esta receta</p>
                  <p className="text-sm text-neutral-400">Agrega ingredientes de tu inventario</p>
                </CardContent>
              </Card>
            ) : (
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            >
              {updateRecipeMutation.isPending ? "Guardando..." : "Guardar Cambios"}
            </Button>
          </div>
        </form>
      </DialogContent>
